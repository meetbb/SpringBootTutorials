...
A: Add a cache (e.g., Redis) in front of the redirect lookup; writes still go straight to the database since they're comparatively rare.

**Q: How would you scale this to millions of requests per day?**
A: Cache hot lookups, add a read replica, consider a CDN/edge redirect layer, and horizontally scale app instances behind a load balancer.
...
**Q: Why H2, and what's the risk of using it here?**
A: In-memory H2 is fast for local development and iteration. The risk: all data is lost on restart, so it's not viable for production as-is — would swap for a persistent database like Postgres or MySQL.
...
## 10. What would you do differently / what's next?

- Randomize short codes to remove guessability.
- Swap H2 for a persistent production database.
- Add caching for the read (redirect) path.
...
**Tip:** the strongest impression comes from the design-decision questions (sections 2–6) — that's where you're tested on understanding *why* the code is shaped the way it is, not just that it works. Sections 9–10 are also good to raise proactively, since naming known gaps yourself reads as self-awareness rather than a weakness.
